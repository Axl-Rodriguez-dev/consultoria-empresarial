document.addEventListener('DOMContentLoaded', () => {
    fetch('./data/enfoque_integral.json')
        .then(res => res.json())
        .then(data => {
            const srv = data.find(item => item.id === "service-3");
            if (srv) renderEnfoqueIntegral(srv);
        })
        .catch(err => console.error("Error cargando enfoque integral:", err));
});

function renderEnfoqueIntegral(srv) {
